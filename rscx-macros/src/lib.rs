...
// use syn::{braced, bracketed, parenthesized, parse::Parse, ItemStruct, Token};
...
//     let nodes = syn::parse_macro_input!(input as Node);
//     quote! { #nodes }.to_token_stream().into()
// }
//
...
//         let nodes = input
//             .parse_terminated(|node| node.parse::<Node>(), Token![,])?
//             .into_iter()
//             .collect::<Vec<_>>();
//         Ok(Nodes(nodes))
//     }
...
//             Ok(Node::TextNode(text))
//         } else if input.peek(syn::Ident) {
//             input
//                 .parse::<Element>()
//                 .map(|element| Node::ElementNode(element))
//         } else if input.peek(syn::token::Paren) {
//             let content;
//             parenthesized!(content in input);
//             content.parse::<syn::Expr>().map(|expr| Node::Expr(expr))
//         } else {
//             Err(input.error("expected string or identifier"))
//         }
//     }
// }
//
...
//     fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
//         let q = match self {
//             Node::TextNode(text) => quote! {
//                 text_node(#text)
//             },
//             Node::ElementNode(element) => quote! { #element },
...
// struct Element {
//     name: syn::Ident,
//     props: HashMap<syn::Ident, syn::Expr>,
//     attributes: Vec<(String, syn::Expr)>,
//     children: Vec<Node>,
// }
//
...
//             parenthesized!(content in input);
//             enum AttrKind {
//                 Prop(syn::Ident, syn::Expr),
//                 Attributes(Vec<(String, syn::Expr)>),
//             }
//             let attrs = content.parse_terminated(
//                 |attr| {
//                     let name = attr.parse::<syn::Ident>()?;
//                     attr.parse::<Token![=]>()?;
//
//                     match name.to_string().as_str() {
//                         "attributes" => {
//                             let attributes_content;
//                             bracketed!(attributes_content in attr);
//
//                             let attrs = attributes_content.parse_terminated(
//                                 |attr| {
//                                     let mut attr_name = String::new();
//                                     while !attr.is_empty() && !attr.peek(syn::Token![=]) {
//                                         if let Ok(name) = attr.parse::<syn::Ident>() {
//                                             attr_name.push_str(name.to_string().as_str());
//                                         } else if let Ok(_) = attr.parse::<Token![-]>() {
//                                             attr_name.push('-');
//                                         } else {
//                                             return Err(attr.error("expected identifier or `-`"));
//                                         }
//                                     }
//                                     attr.parse::<Token![=]>()?;
//                                     let value = attr.parse::<syn::Expr>()?;
//                                     Ok((attr_name, value))
//                                 },
//                                 Token![,],
//                             )?;
//
//                             Ok(AttrKind::Attributes(attrs.into_iter().collect::<Vec<_>>()))
//                         }
//                         _ => {
//                             let value = attr.parse::<syn::Expr>()?;
//                             Ok(AttrKind::Prop(name, value))
//                         }
//                     }
//                 },
//                 Token![,],
...
//             for attr in attrs {
//                 match attr {
//                     AttrKind::Prop(name, value) => {
//                         props.insert(name, value);
//                     }
//                     AttrKind::Attributes(attrs) => {
...
//             content
//                 .parse_terminated(|child| child.parse::<Node>(), Token![,])?
//                 .into_iter()
//                 .collect::<Vec<_>>()
//         } else {
//             vec![]
...
//             &format!("{}Props", name.to_string().to_case(Case::Pascal)),
...
//             chain.push(quote! { .children(Node::Fragment(vec![#(#children),*])) });
//         }
//
//         chain.push({
//             self.props
...
//                 })
//                 .collect::<Vec<_>>();
//             quote! { .attrs(Attributes(vec![#(#attrs),*])) }
//         });
//
